  ...
  return (
    <div className={`${props.className} relative`}>
      ...
      >
        ...
        <div
          className={`w-full paper h-12 px-4 text rounded-md bg-gray-50 
           hover:bg-gray-100 focus:bg-gray-100 ${sOption ? 'shadow-inner' : 'shadow-inner'}
           outline-none focus:outline-none 
           transition-all duration-100 ${props.label ? 'pt-5 pb-1.5' : 'py-3'}`}
          tabIndex={0}
          onBlur={() => toggleForced(false)}
          onClick={() => toggleForced(true)}
        >
          {props.options.find((v) => props.model.bind.value === v.id)?.value ||
            ''}
        </div>
      </div>
      <div
      ...
      >
        {sOption ? (
          <>
            <div className={'h-6 text-xs px-3 text-gray-600 bg-gray-100 py-1 select-none'} key={'label'}>
